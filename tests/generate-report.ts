...
function parseResults(): TestResult[] {
  ...
  function processSuite(suite: any, category: string, projectName: string) {
    if (suite.specs) {
...
function generateHTML(results: TestResult[]): string {
  ...
  const icon = (s: string) =>
    ({ passed: '✅', failed: '❌', skipped: '⏭️', flaky: '⚠️' }[s] || '❓');
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .stat.r .stat-num{color:${passRate >= 80 ? '#34d399' : passRate >= 60 ? '#fbbf24' : '#f87171'}}
...
console.log('='.repeat(50));
console.log('\nOpen report: explorer tests\\report\\dashboard.html\n');
