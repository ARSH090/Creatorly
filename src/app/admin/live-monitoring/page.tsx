        ...
        <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <Card className="border-none shadow-lg bg-white">
                    ...
                    <CardContent>
                        <div className="text-4xl font-black text-slate-900">₹{metrics.revenue.toLocaleString()}</div>
                    ...
                    <CardContent className="space-y-4">
                        {metrics.errorRate > 0.1 && (
                            <div className="p-4 bg-red-100 border-l-4 border-red-500 text-red-900 font-medium rounded animate-pulse">
                                ⚠️ CRITICAL: Error rate has crossed launch threshold ({metrics.errorRate}%). Check Sentry immediately.
                            </div>
                        )}
                        {metrics.apiLatency > 300 && (
                            <div className="p-4 bg-amber-100 border-l-4 border-amber-500 text-amber-900 font-medium rounded">
                                ⚠️ WARNING: API latency is high ({metrics.apiLatency}ms). Verifying DB connection pool.
                            </div>
                        )}
                        <div className="p-4 bg-emerald-100 border-l-4 border-emerald-500 text-emerald-900 font-medium rounded">
                            ✅ Infrastructure: All systems green. MongoDB (15ms), Redis (3ms), Meta API (Online).
                        </div>
                    ...
                    <CardContent className="p-8">
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="text-5xl font-black">₹{(metrics.revenue * 0.4).toLocaleString()}</div>
                            </div>
