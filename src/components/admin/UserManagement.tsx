...
import {
    ...
    ShieldOff, Loader2, AlertCircle, Download
} from 'lucide-react';
...
export default function UserManagement() {
    ...
    const handleExport = async () => {
        try {
            toast.loading('Generating export...', { id: 'exportToast' });
            const res = await fetch(`/api/admin/export/users?search=${search}&role=${filter === 'all' ? '' : filter}`);
            if (res.ok) {
                ...
                const error = await res.json();
                toast.error(error.error || 'Export failed', { id: 'exportToast' });
            }
        } catch (err) {
            console.error('Export failed:', err);
            toast.error('Network error during export', { id: 'exportToast' });
        }
    };
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        <Button
                            ...
                            variant="outline"
                            className="bg-white/5 border-white/5 hover:bg-white/10 text-white font-bold uppercase tracking-widest text-[8px] px-6 h-10 rounded-xl"
                        >
                            <Download className="w-3 h-3 mr-2 text-indigo-400" />
                            Export CSV
                        </Button>
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="bg-black/40 border border-white/5 rounded-2xl pl-12 pr-6 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500/50 transition-all w-64 placeholder:text-zinc-700 font-medium"
                            />
                        </div>
                        <select
                            value={filter}
                            onChange={(e) => setFilter(e.target.value)}
                            className="bg-black/40 border border-white/5 rounded-2xl px-6 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500/50 transition-all font-bold uppercase tracking-widest"
                        >
                                ...
                                <td className="px-8 py-6 text-right">
                                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        {user.isSuspended ? (
                                            ...
                                            <button
                                                onClick={() => handleAction(user._id, 'suspend', 'Violated terms of service')}
                                                disabled={actionLoading === user._id}
                                        ...
                                        {user.payoutStatus === 'held' ? (
                                            <button
                                                onClick={() => handleAction(user._id, 'unfreeze_payout')}
                                                disabled={actionLoading === user._id}
                                            ...
                                            <button
                                                onClick={() => handleAction(user._id, 'freeze_payout', 'Unusual financial activity')}
                                                disabled={actionLoading === user._id}
