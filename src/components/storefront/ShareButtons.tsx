...
export default function ShareButtons({ username, displayName }: ShareButtonsProps) {
    ...
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out ${displayName}'s store on Creatorly!`)}&url=${encodeURIComponent(profileUrl)}`;
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`Check out ${displayName}'s store on Creatorly! ${profileUrl}`)}`;
