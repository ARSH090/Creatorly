        ...
        <DialogContext.Provider value={{ open: currentOpen, setOpen: handleOpenChange }}>
            ...
            {currentOpen && (
                <div className="fixed inset-0 z-50 flex items-start justify-center sm:items-center">
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm transition-all duration-100 animate-in fade-in-0" onClick={() => handleOpenChange(false)} />
                </div>
            )}
...
export const DialogTrigger = ({ asChild, children }: { asChild?: boolean, children: React.ReactNode }) => {
    ...
    // Specific handling for standard HTML elements to avoid cloning issues if possible
    // But for simplicity and fixing the error, we just wrap in a span that handles the click.
    // This avoids all cloneElement complexities.
    // If asChild is true, people usually expect the child to be the trigger.
    // But wrapping it is safer for now.
