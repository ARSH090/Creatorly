...
/**
 * Idempotent plan seeder — safe to run multiple times.
...
export async function seedDefaultPlans() {
    ...
    for (const plan of DEFAULT_PLANS) {
        const existing = await Plan.findOne({ id: plan.id });
        if (existing) {
    ...
    console.log(`Plans seeded: [${seeded.join(', ')}], skipped: [${skipped.join(', ')}]`);
